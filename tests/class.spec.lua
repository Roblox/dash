return function()
	...
	describe("class", function()
		...
		it("can extend a class with independent methods", function()
			...
			assertSnapshot(tostring(car), [["Vehicle"]])
			assertSnapshot(car:getWheelCount(), [[4]])
			assertSnapshot(typeof(car.drive), [["nil"]])
			assertSnapshot(typeof(bike.drive), [["function"]])
		...
		it("can use isInstance to detect for class membership", function()
			...
			local bike = Bike.new()
			local redBike = RedBike.new()
			assertSnapshot(RedBike.isInstance(redBike), [[true]])
			assertSnapshot(Bike.isInstance(redBike), [[true]])
			assertSnapshot(Vehicle.isInstance(redBike), [[true]])
			assertSnapshot(RedBike.isInstance(bike), [[false]])
		end)
	end)
end
